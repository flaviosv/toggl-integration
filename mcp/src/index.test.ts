...
import { makeTmpCachePath } from "./tools/test-harness.js";
...
test("missing TOGGL_API_TOKEN: non-zero exit, stderr names the variable, zero bytes on stdout", async () => {
  ...
  try {
    const child = spawn(process.execPath, [ENTRY_POINT], {
      env: { PATH: process.env.PATH ?? "", TOGGL_WORKSPACE_ID: "99", TOGGL_CACHE_PATH: cachePath },
    ...
    fsSync.rmSync(path.dirname(cachePath), { recursive: true, force: true });
  }
});
