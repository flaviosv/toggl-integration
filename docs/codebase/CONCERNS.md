...
**`mcp` — subprocess-exit not verified after client disconnect (deliberately left open):** `mcp/src/index.test.ts`'s happy-path MCP-handshake test only calls `client.close()` on the `StdioClientTransport` and never confirms the spawned `dist/index.js` process actually exits on stdin EOF. If the server fails to tear down cleanly, this test would still pass while silently leaking an orphaned Node process per run. Flagged in code review (finding `I5`, `.specs/features/TOGGL-2-time-entries-mcp/fix-code-review.md`) and explicitly left unresolved during fix-review — no documented `@modelcontextprotocol/sdk` API was found to assert child-process exit cleanly, and the team chose a deliberate follow-up decision over a speculative fix. Priority: low (test-only gap, not a runtime defect). Fix approach: once an SDK-supported way to await/assert transport-level process teardown is confirmed, add the assertion to the existing test.
